...
```typescript
const DEFAULT_MAX_RETRIES = 3;           // BullMQ attempts = 总尝试次数
...
await this._queue.add(
  ...
  {
    jobId: options.id,
    attempts: DEFAULT_MAX_RETRIES,        // attempts=3 最终语义：总尝试 3 次 = 首次 1 次 + 重试 2 次
    backoff: {
...
- ✅ 重新经过完整的队列重试机制（attempts=3: 首次 1 次 + 重试 2 次）
- ✅ 创建新的 WebhookCall 记录, 不覆盖历史
- ❌ 不保留原始请求 ID 关联

### 3.3 自动重试 vs 手动重发对比（attempts=3 口径完全统一）

| 维度 | 自动重试 | 手动重发 |
|------|----------|----------|
| **触发者** | BullMQ 队列调度 | 管理员通过 UI 触发 |
| **attempts=3 统一语义** | 总尝试 3 次 = 首次 1 次 + 重试 2 次 | 新任务同样 attempts=3 配置，语义完全一致 |
| **重试间隔** | 指数退避 (1s → 2s) | 立即执行 + 新的指数退避 (1s → 2s) |
...
| **判断依据** | `job.attemptsMade >= attempts - 1` 代码逻辑 | 统一 BullMQ 机制 |
...
### 6.2 写入时机与事务边界（最终三类逐调用点核查）

#### 分类 A：事务内回滚（业务操作与日志写入原子绑定）
| 调用文件与位置 | 操作场景 | 失败后果 |
|--------------|----------|---------|
| `complete-document-with-token.ts:293-321` | 收件人信息更新（姓名/邮箱变更日志） | prisma.$transaction 内写入，失败触发整体事务回滚 |
| `complete-document-with-token.ts:329-346` | 收件人完成签名日志 | prisma.$transaction 内写入，失败触发整体事务回滚 |
| `seal-document.handler.ts:285-287` | 文档完成封存日志 | prisma.$transaction 内写入，失败触发整体事务回滚 |
| `viewed-document.ts:69-86` | 文档首次打开日志 | prisma.$transaction 内写入，失败触发整体事务回滚 |

#### 分类 B：直接抛错中断（无 try-catch 保护，失败即终止流程）
| 调用文件与位置 | 操作场景 | 失败后果 |
|--------------|----------|---------|
| `complete-document-with-token.ts:145-155` | 2FA 认证失败日志 | 无 try-catch，写入失败抛出异常终止后续签名流程 |
| `complete-document-with-token.ts:162-172` | 2FA 认证成功日志 | 无 try-catch，写入失败抛出异常终止后续签名流程 |
| `complete-document-with-token.ts:237-260` | 字段自动插入批量日志 | 无 try-catch，createMany 失败抛出异常 |
| `viewed-document.ts:31-48` | 文档查看日志（首次查看） | 无 try-catch，写入失败抛出异常终止查看流程 |
| `send-completed-email.ts:152-167` | 邮件发送日志（文档所有者） | 无 try-catch，写入失败抛出异常终止邮件发送流程 |
| `send-completed-email.ts:225-240` | 邮件发送日志（收件人） | 无 try-catch，写入失败抛出异常终止邮件发送流程 |
| `send-2fa-token-email.ts:123-133` | 2FA 邮件发送成功日志 | 无 try-catch，写入失败抛出异常 |

#### 分类 C：catch 吞错继续
| 调用文件与位置 | 操作场景 | 失败后果 |
|--------------|----------|---------|
| **最终代码核查结论** | 全量检索 28 个审计日志写入文件 | ❌ 不存在此类调用，所有写入均无独立 try-catch 保护 |

**最终核查结论**:
- ✅ 事务内回滚: 4 处（核心状态变更场景）
- ✅ 直接抛错中断: 7 处（含安全认证、邮件发送、字段操作等场景）
- ❌ catch 吞错继续: 0 处（经全量核查确认）
...
1. **审计强一致原则**
   关键业务操作的审计日志写入与业务状态更新绑定在同一事务中，日志写入失败则业务操作一并回滚。非事务场景的日志写入无 try-catch 保护，失败直接中断流程。

2. **尽力而为集成原则**
   Webhook 采用异步非阻塞模式，触发失败不影响已完成业务流程，通过 BullMQ 自动重试 + 管理员手动重发双层机制保障最终投递。
...
| 指标 | 值 | 说明 |
|------|---|------|
| **Webhook attempts 统一语义** | 3 | BullMQ attempts=3 = **总尝试 3 次 = 首次 1 次 + 重试 2 次** |
| **重试延迟策略** | 指数退避 | 1s → 2s |
...
| **审计日志事务内回滚** | 4 处 | 收件人更新、收件人完成、文档封存、文档首次打开 |
| **审计日志直接抛错** | 7 处 | 2FA 认证成败、字段插入、文档查看、邮件发送×2、2FA邮件日志 |
| **catch 吞错继续** | 0 处 | 全量检索 28 个写入文件，未发现此类调用 |
| 审计日志事件类型 | 30+ 种 | 覆盖文档全生命周期 |

---
**报告生成时间**: 2026-05-16
**分析版本**: Documenso v1.0+
**最终修正记录**: 2026-05-16 基于全量代码核查完成最终定点校正：
1. ✅ 统一 attempts=3 语义口径（自动重试、手动重发、关键数字三处完全一致）
2. ✅ 按三类逐调用点列出审计日志失败后果（事务内回滚 4 处、直接抛错中断 7 处、catch 吞错继续 0 处）
3. ✅ 删除与代码证据冲突的总结语句（审计优先策略、非关键操作异步等）
4. ✅ 修正重试延迟表述（1s→2s，删除不存在的 4s 延迟）
