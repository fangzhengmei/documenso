...
PDF规范化 (flatten + 1.7升级) → seal-document.handler.ts:357
    ↓
字段插入 (V1/V2两种模式) → seal-document.handler.ts:381-452
    ↓
...
| V2 | `insert-field-in-pdf-v2.ts` | Konva画布直接渲染 → PDF覆层合并 | `seal-document.handler.ts:402-452` |
...
PDF.load() 加载覆层文档 → seal-document.handler.ts:423
    ↓
embedPage 嵌入主文档页面 → seal-document.handler.ts:423
    ↓
drawPage 绘制到目标页面（处理旋转适配） → seal-document.handler.ts:445-451
```
...
│  transports/local.ts│                 │ transports/google-cloud.ts │
...
> 本项目采用标准数字签名架构，签名参数配置见 `packages/signing/index.ts:43-52`，通过 `pdf.sign()` 方法将数字签名嵌入PDF文档。

#### 通用推断（带前提条件）

> 🔍 **通用推断**：标准数字签名实现包含以下防篡改特性。前提条件：底层签名算法和实现正确。
>
> - 文档哈希嵌入签名中：签名计算涵盖文档内容哈希
> - 签名范围覆盖整个PDF文档（除签名字典本身的增量更新区域）
> - 任何对文档的修改（不包括合法的增量更新）都会导致签名验证失败
...
- **私钥控制**：
  - 本地模式：`packages/signing/transports/local.ts:5-23` - 从用户配置加载P12证书
  - HSM模式：`packages/signing/transports/google-cloud.ts` - 使用Google Cloud HSM保护私钥
- **时间戳集成**：`packages/signing/index.ts:49` - 集成TSA时间戳服务器
- **审计日志附加**：`packages/lib/jobs/definitions/internal/seal-document.handler.ts:199-245` - 完整审计日志附加到最终文档
...
| 标记类型 | 视觉标识 | 含义 | 使用规范 |
|---------|---------|------|---------|
| **仓内可证据** | 标注文件:行号 | 代码仓库中有明确实现可查证 | 所有代码位置精确到文件和行号范围 |
| **明确推断标记** | ⚠️ + 注释包裹 | 基于代码使用模式的合理推断 | 使用 `// ── 推断开始/结束 ──` 明确边界 |
| **通用推断** | 🔍 + 前提条件 | 行业标准或常见做法，非项目特定代码 | 必须标注前提条件，说明适用边界 |

### v5 修正记录

1. **代码行号准确性修正**：
   - drawPage 调用位置：从 `seal-document.handler.ts:464-470` 修正为 `seal-document.handler.ts:445-451`
   - 字段插入循环范围：从 `seal-document.handler.ts:381-453` 修正为 `seal-document.handler.ts:381-452`
   - signPdf 调用位置：从 `seal-document.handler.ts:461` 确认为正确位置

2. **文件路径拼写修正**：
   - 架构图中 `transports/g-c.ts` 修正为 `transports/google-cloud.ts`
   - 时间戳集成引用添加缺失的反引号
