...
| 层级 | 坐标类型 | 单位 | 原点 | 说明 |
|------|---------|------|------|------|
| 1 | 浏览器屏幕坐标 (clientX/Y) | 像素 | 浏览器视口左上角 | 鼠标在视口中的绝对位置 |
| 2 | 文档坐标 (pageX/Y) | 像素 | 文档左上角 | 鼠标相对于整个文档的位置（含滚动偏移） |
| 3 | 页面相对 DOM 坐标 | 像素 | DOM 页面元素左上角 | 鼠标/元素相对于 PDF 页面 DOM 元素的位置 |
| 4 | Konva 绝对坐标（显示坐标） | 像素 | Stage 容器左上角 | Konva 拖拽回调 pos.x/pos.y、group.absolutePosition()、getClientRect() 坐标（已包含所有父级 scale 变换） |
| 5 | Konva 逻辑坐标 | 像素 | Stage 内部坐标系 | group.x()/group.y() 获取的值（未缩放，元素存储的原始坐标） |
| 6 | 百分比坐标 | % (0-100) | PDF页面左上角 | 相对于页面宽高的百分比（最终数据库存储格式） |

> **关键关系：`pos.x = group.absolutePosition().x = group.x() * scale
>
> 详细语义参考：coordinate-semantic-consistency.md
