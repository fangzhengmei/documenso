...
**`\S` 定义**（基于 ECMAScript 规范）：

| 元字符 | 语义 |
|--------|------|
| `\s` | 匹配所有空白字符，包括：` `（空格）、`\t`（制表符）、`\n`（换行）、`\r`（回车）、`\v`（垂直制表符）、`\f`（换页符）、以及 Unicode 空白字符（如 `\u00A0` NO-BREAK SPACE 等） |
| `\S` | `\s` 的补集，匹配任何非空白字符，等价于 `[^\s]` |

**关键点**：`\S` 的语义由 ECMAScript 正则引擎定义，涉及 Unicode 属性匹配（`\p{White_Space}`），**不可**简单等价于 `[^\r\n\t\f\v ]`。

**不被识别为占位符的情况**：
| 输入 | 原因 | 是否匹配 |
|------|------|---------|
| `{name age}` | 中间有空格（属于 `\s`） | ❌ 不匹配 |
| `{name\nage}` | 中间有换行（属于 `\s`） | ❌ 不匹配 |
| `{name\tag}` | 中间有制表符（属于 `\s`） | ❌ 不匹配 |
| `{ name}` | 左花括号后有空格（属于 `\s`） | ❌ 不匹配 |
| `{name }` | 右花括号前有空格（属于 `\s`） | ❌ 不匹配 |
| `{}` | 空的花括号（`\S+` 要求至少一个字符） | ❌ 不匹配 |
| `{signer.name}` | 点号不属于 `\s` | ✅ 匹配（key = `signer.name`） |
| `{document_name}` | 下划线不属于 `\s` | ✅ 匹配（key = `document_name`） |
...
**基于代码事实的行为分析**：
- 采用"**静默回退**"策略，不抛异常、不报错
- 未命中时返回 `key`（变量名）而非原始 `{key}`
- `\S+` 的选择意味着**不支持**包含 `\s` 的 key（空格、换行、制表符等）
- 双花括号 `{{name}}` 的行为是**贪婪匹配**的自然结果（`{` 和 `}` 之间的 `\S+` 会尽可能多地匹配）
- 属于"尽力而为"（best-effort）的字符串替换模式
...
### 本次修订要点（2026-05-11，第 3 次修订）

7. **`\S` 语义校准**：
   - 原表述：`\S` 等价于 `[^\r\n\t\f\v ]`
   - 修正后：`\S` 是 `\s` 的补集（等价于 `[^\s]`），`\s` 包含 Unicode 空白字符（如 `\u00A0` 等），**不可**等价于 `[^\r\n\t\f\v ]`
   - 依据：ECMAScript 正则表达式规范

8. **删除无代码依据的意图推断**：
   - 删除："这是有意的约束"、"便于识别哪些变量未被替换"、"副作用"等主观推断
   - 修正为：基于代码事实的行为描述，如 "静默回退"、"贪婪匹配的自然结果"

---

### 历史修订要点（2026-05-11，第 2 次修订）
