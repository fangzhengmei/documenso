...
本文档详细分析 Documenso 中 PDF 字段的四种状态（编辑态、存储态、文档发送阶段自动回填、签署提交阶段）的数据结构及其相互转换逻辑。
...
### 关键限制条件
1. **仅 V2 信封生效**: `envelope.internalVersion === 2`
2. **仅未发送的接收者**: `recipient.sendStatus !== SendStatus.SENT`
...
**文件**: `packages/trpc/server/envelope-router/sign-envelope-field.types.ts`
...
> **重要注意**: `FREE_SIGNATURE` 类型未在 tRPC 签署请求 schema 中定义，但在 `signFieldWithToken` 旧函数中支持。
...
### 核心函数
**函数名**: `renderField`  
**文件**: `packages/lib/universal/field-renderer/render-field.ts`
...
| **FREE_SIGNATURE** | `""` (空字符串) | 从 `field.signature` 关联读取 | ❌ 抛出错误 "Free signature fields are not supported" |

> **注意**: `FREE_SIGNATURE` 在新版渲染器中不受支持，仅在 V1 遗留 API 中使用。
...
| `fromCheckboxValue` | `packages/lib/universal/field-checkbox.ts:1` | 存储态 → 验证态 | `string` → `number[]` |
| `toCheckboxValue` | `packages/lib/universal/field-checkbox.ts:19` | 值数组 → 存储态 | `string[]` → `string` (JSON) |
...
### 4. fromCheckboxValue 行为修正
- **⚠️ 文档之前错误**: 错误描述为返回 `string[]` 值数组
- **✅ 实际行为**: 返回 `number[]` 索引数组
- **正确用法**: 返回的是索引，需要结合 `fieldMeta.values` 才能映射到文本值
...
| 复选框值转换 (验证用) | `packages/lib/universal/field-checkbox.ts` |
